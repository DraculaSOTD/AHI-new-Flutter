...
class RppgCommon {
  ...
  /// Check if the native SDK singleton is fully initialized
  /// Returns true if all required components (socket, camera, etc.) are ready
  /// This should be checked before calling configure/startVideo/startAnalysis
  Future<bool> isInitialized() => RppgCommonPlatform.instance.isInitialized();
  ...
  /// Stops video capturing and releases camera resources.
  /// Should be invoked when you need to stop the video session.
  /// Will change `state` to `prepared` in case of success.
  Future<void> stopVideo() => RppgCommonPlatform.instance.stopVideo();
  ...
  /// Destroy camera manager and release camera resources
  Future<void> destroyCamera() => RppgCommonPlatform.instance.destroyCamera();

  /// Reset Analysis singleton for new scan session
  /// Recreates camera view and resets all state to allow consecutive scans
  Future<void> resetForNewScan() => RppgCommonPlatform.instance.resetForNewScan();
}
